...
If one of the backend health-checks is marked as unhealthy like in the screenshot above, it means you didn't create "vnet-customer" V-Net as described in the :ref:`"Learn by Creating Services"<s12-e-bgp>` manual. If that's the case, let's create it from Kubernetes using the V-Net custom resource.
...
BTW, if you already created "vnet-customer" V-Net as described in the :ref:`"Learn by Creating Services"<s12-e-bgp>`, you may import that to k8s, by adding ``resource.k8s.netris.ai/import: "true"`` annotation in V-Net manifest, the manifest should look like this:
...
Let's create a new BGP peer, that is listed in the :ref:`"Learn by Creating Services"<s12-learn-by-doing>`.
...
NAME                      STATE     BGP STATE                                          PORT STATE   NEIGHBOR AS   LOCAL ADDRESS      REMOTE ADDRESS     AGE
iris-isp2-ipv4-customer   enabled   bgp: Established; prefix: 957240; time: 00:04:02                65007         45.38.161.121/30   45.38.161.126/30   2m3s
