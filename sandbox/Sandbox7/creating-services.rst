...
* In a web browser: (*\*Fields not specified should remain unchanged and retain default values*)
  ...
  7. For the **VLAN ID** field, uncheck the **Untag** check-box and type in ``1072``.
...
* In a web browser: (*\*Fields not specified should remain unchanged and retain default values*)
  ...
  11. From the **Select IP** drop-down menu, select the "**50.117.59.164/32**" IP address.
    ...
    * This public IP is part of **50.117.59.164/30 (NAT)** subnet which is configured in the **NET → IPAM** section with the purpose of **NAT** and indicated in the SoftGate configurations to be used as a global IP for NAT by the :ref:`"Netris SoftGate Agent"<netris_sg_agent>`..
...
* In a web browser: (*\*Fields not specified should remain unchanged and retain default values*)
  ...
  1. Log into the Netris Controller by visiting `https://sandbox7.netris.ai <https://sandbox7.netris.ai>`_ and navigate to **Services → Instances(ROH)**.
