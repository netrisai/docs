...
===================================
Once you log into the Netris GUI by visiting **https://demo55.netris.ai** and navigating to **Services > V-Net**, you will find a V-Net service named "**V-Net Example**" already configured for you as an example. You can examine the particular service settings by selecting **Edit** from the **Actions** menu indicated by three vertical dots (⋮) on the right side of the "**V-Net Example**" service.
...
2. Select the **sw01-nyc(10.254.36.1)** switch from the **Select switch** drop-down menu.
3. Select the **Ping** radio button from the row of available choices.
4. Type in ``192.168.35.64`` in the field labeled "**IPv4 address**".
5. Click **Submit**.
...
.. code-block:: shell-session
  ...
  sw01-nyc# ping -c 5 192.168.35.64
  PING 192.168.35.64 (192.168.35.64) 56(84) bytes of data.
  64 bytes from 192.168.35.64: icmp_seq=1 ttl=64 time=33.4 ms
  64 bytes from 192.168.35.64: icmp_seq=2 ttl=64 time=27.1 ms
  64 bytes from 192.168.35.64: icmp_seq=3 ttl=64 time=0.702 ms
  64 bytes from 192.168.35.64: icmp_seq=4 ttl=64 time=1.37 ms
  64 bytes from 192.168.35.64: icmp_seq=5 ttl=64 time=0.609 ms
  --- 192.168.35.64 ping statistics ---
