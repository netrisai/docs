...
After installation use ``EXTERNAL-IP`` of haproxy service as ``--controller`` parameter in `netris-setup <https://docs.netris.ai/en/stable/switch_agent_installation.html#install-the-netris-agent>`_

.. code-block::

   kubectl get svc -nnetris-controller |grep haproxy

and ``$mystrongauthkey`` as ``--auth`` parameter in `netris-setup <https://docs.netris.ai/en/stable/switch_agent_installation.html#install-the-netris-agent>`_
